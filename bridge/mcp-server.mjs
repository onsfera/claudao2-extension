...
const TOOLS = [
  ...
  // --- Ação ---
  { name: "browser_click", cmd: "click", description: "Clica no elemento: por 'selector' CSS, 'ref' (de browser_snapshot) ou 'text'. Rola até ele, espera visível, espera spinner sumir, e move o cursor animado. Retorna 'changed'/'navigated' (o clique teve efeito?) e 'errors' (mensagens de validação que apareceram). Use real=true para eventos de mouse REAIS via CDP.", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" }, ref: { type: "string" }, text: { type: "string" }, real: { type: "boolean", description: "clique com eventos reais de mouse (CDP Input)" } } } },
