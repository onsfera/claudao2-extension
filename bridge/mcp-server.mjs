...
// Timeout por comando: alguns (eval/screenshot/navigate/upload/login/wait/type) legitimamente
// demoram mais que os 25s padrão. Se o comando trouxer timeoutMs explícito, respeita-o (+buffer
// de round-trip). Teto de 10min.
function cmdTimeoutMs(cmd, args) {
  if (args && Number(args.timeoutMs) > 0) return Math.min(600000, Number(args.timeoutMs) + 8000);
  // type digita char-a-char (dispara typeaheads) → escala com o tamanho do texto p/ não cortar no meio.
  if (cmd === "type" && args && args.text) {
    const per = (Number(args.delay) > 0 ? Number(args.delay) : 25) + 12; // ms/char (digitação + eventos)
    return Math.min(600000, 20000 + String(args.text).length * per);
  }
  const perCmd = { eval: 38000, screenshot: 45000, navigate: 40000, upload: 40000, login: 40000, wait: 35000, type: 40000 };
  return perCmd[cmd] || 25000;
}
// --- HUB: encaminha comandos (próprios e dos editores clientes) à extensão ---
function forwardToExtension(cmd, args, client, editorConn, rid, resolve, timeoutMs) {
...
function dispatchHub(cmd, args, client) { return new Promise((resolve) => forwardToExtension(cmd, args, client, null, null, resolve, cmdTimeoutMs(cmd, args))); }
...
function onUpgrade(req, socket) {
  ...
  if (isEditor) {
    ...
    conn.onMessage = (data) => { let m; try { m = JSON.parse(data); } catch { return; } if (m.type === "hello") return; if (m.cmd) forwardToExtension(m.cmd, m.args, m.client, conn, m.rid, null, cmdTimeoutMs(m.cmd, m.args)); };
...
function dispatchClient(cmd, args, client) {
  return new Promise((resolve) => {
    ...
    const rid = ++crid;
    const timer = setTimeout(() => { clientPending.delete(rid); resolve({ ok: false, error: "Tempo esgotado no comando '" + cmd + "'." }); }, cmdTimeoutMs(cmd, args));
...
const TOOLS = [
  ...
  { name: "browser_console", cmd: "console", description: "Logs de console capturados desde o carregamento: console.log/info/debug MAIS exceções não-capturadas e promise rejections (nível 'error'). Filtre por nível. Obs.: console.warn/error EXPLÍCITOS da página não são espelhados aqui de propósito (evita poluir a lista de erros da extensão); para falhas de rede use browser_network.", inputSchema: { type: "object", properties: { tabId: TABID, limit: { type: "number" }, level: { type: "string", enum: ["log", "info", "warn", "error", "debug"] } } } },
  ...
  { name: "browser_eval", cmd: "eval", description: "Executa JavaScript na página (via chrome.debugger) e retorna o resultado. Aceita await (top-level). Funciona em abas de fundo. Para promises LONGAS (>30s), passe timeoutMs (o padrão é 30s; teto ~10min). Dica p/ operações muito longas: guarde o resultado num window global no fim do código e reconsulte depois com outro eval.", inputSchema: { type: "object", properties: { tabId: TABID, code: { type: "string" }, timeoutMs: { type: "number", description: "tempo máx. de espera do resultado (padrão 30000; use maior p/ promises longas)" } }, required: ["code"] } },
  ...
  { name: "browser_type", cmd: "type", description: "Digita texto caractere a caractere com eventos de teclado REAIS (keydown/beforeinput/input/keyup) no elemento (selector/ref) ou no focado. É a forma certa de disparar typeaheads (menções @ do LinkedIn, autocompletes) e de escrever em editores rich text (Quill/contenteditable): detecta contenteditable e insere via execCommand + faz um keystroke de sincronização no fim (espaço+backspace) pra o editor não perder o texto ao re-renderizar. Ignora o input-fantasma offscreen. 'delay' ajusta o intervalo entre teclas (padrão 60ms; útil pra typeahead pegar). Retorna 'value' (o que ficou no campo) e 'target'. Para menções: digite o gatilho, depois selecione a opção do dropdown NA MESMA sequência de ações (o overlay fecha entre chamadas).", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" }, ref: { type: "string" }, text: { type: "string" }, delay: { type: "number", description: "ms entre teclas (padrão 60; aumente se o typeahead não acompanhar)" }, syncEditor: { type: "boolean", description: "em contenteditable, faz o keystroke de sync no fim (padrão true)" } }, required: ["text"] } },
  ...
  { name: "browser_navigate", cmd: "navigate", description: "Navega a ABA ATUAL para uma URL — isso SUBSTITUI a página (você perde formulários/estado dela). Para abrir uma página de referência, recurso ou arquivo SEM perder a página em que está trabalhando, passe newTab:true (ou use browser_open_tab) e continue agindo na aba original. Arquivos/recursos locais (file:/data:/blob:) abrem SEMPRE em nova aba. Quando abre nova aba, retorna o tabId novo. Ao navegar in-place, ESPERA a página carregar (status complete + pequeno settle) antes de retornar — então o snapshot/look seguinte já reflete a página nova, não a anterior (retorna 'load' e a 'url' final). Use timeoutMs para ajustar a espera de carregamento.", inputSchema: { type: "object", properties: { tabId: TABID, url: { type: "string" }, newTab: { type: "boolean", description: "abrir em NOVA aba (preserva a página de trabalho); retorna o tabId novo" }, active: { type: "boolean", description: "ao abrir nova aba, trazer para frente (padrão true; use false para abrir em segundo plano)" }, timeoutMs: { type: "number", description: "espera máx. de carregamento in-place (padrão 15000)" } }, required: ["url"] } },
  ...
  { name: "browser_upload", cmd: "upload", description: "ÚNICA forma correta de anexar arquivo(s) LOCAL(is): coloca-os num <input type=file> direto pelo caminho (via CDP DOM.setFileInputFiles), SEM abrir o seletor de arquivos do sistema. NUNCA clique no botão 'Escolher arquivo/Anexar/Carregar' (browser_click) para subir arquivo: isso abre o diálogo NATIVO do Chrome, que a extensão NÃO controla, trava o agente e obriga o usuário a fechar na mão. Em vez disso: ache o <input type=file> (browser_snapshot/query costumam achá-lo mesmo oculto atrás do botão; se preciso, o input fica logo após/dentro do botão), e passe o 'selector' dele + 'files' (caminhos absolutos) ou 'path'. Funciona com input oculto, em shadow DOM e iframes. Age na aba de trabalho sem trocá-la.", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" }, files: { type: "array", items: { type: "string" } }, path: { type: "string" } }, required: ["selector"] } },
  // --- Multi-aba ---
  { name: "browser_open_tab", cmd: "open_tab", description: "Abre uma NOVA aba na URL dada e retorna o tabId novo — use para páginas/recursos auxiliares sem perder a aba onde está trabalhando. active=false abre em segundo plano (não rouba o foco). group=true põe a nova aba no SEU grupo de abas (cria o grupo se ainda não houver); se você já tem um grupo, a aba entra nele automaticamente.", inputSchema: { type: "object", properties: { url: { type: "string" }, active: { type: "boolean" }, group: { type: "boolean", description: "agrupar a nova aba no seu grupo (default: só se você já tiver grupo)" } }, required: ["url"] } },
  ...
  { name: "browser_tab_group", cmd: "tab_group", description: "Organiza suas abas num GRUPO próprio no Chrome, SOB DEMANDA (quando a tarefa passa a envolver 2+ páginas). action:'create' cria/garante o seu grupo e move as abas indicadas (tabIds; sem tabIds usa a aba ativa) — o grupo nasce nomeado 'Claudão² · <seu projeto>'. action:'add' acopla abas JÁ ABERTAS ao seu grupo (use browser_tabs p/ achar o tabId e ADOTE em vez de duplicar). action:'list' mostra seu grupo e seus membros. action:'ungroup' tira abas do grupo. Não force um grupo pra uma aba só; agrupe ao virar multi-aba.", inputSchema: { type: "object", properties: { action: { type: "string", enum: ["create", "add", "list", "ungroup"], description: "create | add | list | ungroup (default create)" }, tabIds: { type: "array", items: { type: "number" }, description: "abas a agrupar/desagrupar" }, title: { type: "string" }, color: { type: "string", enum: ["grey", "blue", "red", "yellow", "green", "pink", "purple", "cyan", "orange"] } } } },
...
ABAS: você começa na aba atual, sem grupo. Quando a tarefa passar a envolver MAIS DE UMA página, organize com browser_tab_group (grupo sob demanda, não force pra uma aba só). Antes de abrir uma página nova, cheque browser_tabs: se ela JÁ está aberta, ACOPLE com browser_tab_group action:'add' em vez de duplicar. Abra auxiliares com browser_open_tab (group:true p/ já entrar no seu grupo).`;
