...
const TOOLS = [
  ...
  { name: "browser_get_state", cmd: "get_state", description: "Estado da página: url, título, readyState, viewport. Opcional: cookies e chaves de localStorage (redigidos) para depuração.", inputSchema: { type: "object", properties: { tabId: TABID, includeStorage: { type: "boolean" } } } },
  ...
  { name: "browser_fill", cmd: "fill", description: "Preenche input/textarea (por selector ou ref). Dispara input/change. clearFirst limpa antes.", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" }, ref: { type: "string" }, value: { type: "string" }, clearFirst: { type: "boolean" } }, required: ["value"] } },
  ...
  { name: "browser_select", cmd: "select", description: "Seleciona uma opção em um <select> por 'value' ou 'label'.", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" }, value: { type: "string" }, label: { type: "string" } }, required: ["selector"] } },
  { name: "browser_submit", cmd: "submit", description: "Submete um formulário (selector do form ou de um campo dentro dele).", inputSchema: { type: "object", properties: { tabId: TABID, selector: { type: "string" } } } },
